...
async function loadProfileAndRoles(authUserId) {
  ...
  if (isSuperuser || isAdmin) {
    const allPerms = await client_1.db
  ...
  const permRows = await client_1.db
    ...
    .innerJoin(
      schema_1.permissions,
      (0, drizzle_orm_1.eq)(
        schema_1.staffPermissions.permissionId,
        schema_1.permissions.id,
      ),
    )
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.eq)(schema_1.staffPermissions.staffId, staffRow.id),
        (0, drizzle_orm_1.eq)(schema_1.staffPermissions.granted, true),
      ),
    );
...
async function createTRPCContext(opts) {
  ...
  const {
    ...
    staff: staffRow,
    permissionKeys,
  } = await loadProfileAndRoles(authUser.id);
  return {
    req,
    resHeaders,
