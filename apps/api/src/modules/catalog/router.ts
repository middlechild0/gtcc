import { z } from "zod";
import { billableItemTypeEnum, productCategoryEnum } from "@visyx/db/schema";
import { eq, desc, ilike, and } from "drizzle-orm";
import { protectedProcedure, router } from "../../trpc/init";
import { hasPermission } from "../../trpc/middleware/withPermission";
import { TRPCError } from "@trpc/server";
import { db } from "@visyx/db/client";
import { services, products, billableItems } from "@visyx/db/schema";
...
export const catalogRouter = router({
  listServices: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const allServices = await db.query.services.findMany({
        where: (input.search ? ilike(services.name, `%${input.search}%`) : undefined) as any,
        limit: input.limit + 1,
  ...
  createService: protectedProcedure
    ...
    .input(
      z.object({
        name: z.string().min(1),
        category: z.enum(["CONSULTATION", "DIAGNOSTIC", "OPTICAL", "PROCEDURE", "OTHER"]),
    ...
    .mutation(async ({ input }) => {
      ...
      const [newBillableItem] = await db
  ...
  updateService: protectedProcedure
    ...
    .input(
      z.object({
        ...
        category: z.enum(["CONSULTATION", "DIAGNOSTIC", "OPTICAL", "PROCEDURE", "OTHER"]).optional(),
        description: z.string().optional(),
  ...
  listProducts: protectedProcedure
    ...
    .query(async ({ input }) => {
      const allProducts = await db.query.products.findMany({
        where: (input.search ? ilike(products.name, `%${input.search}%`) : undefined) as any,
        limit: input.limit + 1,
  ...
  createProduct: protectedProcedure
    ...
    .input(
      z.object({
        ...
        category: z.enum(["FRAME", "LENS", "CONTACT_LENS", "ACCESSORY", "MEDICATION", "CONSUMABLE", "OTHER"]),
        description: z.string().optional(),
  ...
  updateProduct: protectedProcedure
    ...
    .input(
      z.object({
        ...
        name: z.string().min(1).optional(),
        sku: z.string().optional(),
        category: z.enum(["FRAME", "LENS", "CONTACT_LENS", "ACCESSORY", "MEDICATION", "CONSUMABLE", "OTHER"]).optional(),
        description: z.string().optional(),
        isActive: z.boolean().optional(),
        vatExempt: z.boolean().optional(),
      })
  ...
  listBillableItems: protectedProcedure
    ...
    .query(async ({ input }) => {
      const conditions = [];
      if (input.search) conditions.push(ilike(billableItems.name, `%${input.search}%`));
      if (input.type) conditions.push(eq(billableItems.type, input.type));
      if (input.isActive !== undefined) conditions.push(eq(billableItems.isActive, input.isActive));
