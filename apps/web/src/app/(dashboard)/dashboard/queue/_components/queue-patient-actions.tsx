...
export function QueuePatientActions({ patient }: QueuePatientActionsProps) {
  ...
  const callPatientMut = trpc.queue.callPatient.useMutation({
    onSuccess: () => {
      toast({
        ...
        description: `${patient.patientName} has been called.`,
      });
  ...
  const advancePatientMut = trpc.queue.advanceWorkflow.useMutation({
    onSuccess: () => {
      toast({
        title: "Patient Advanced",
        description: `${patient.patientName} has been moved to the next step.`,
      });
      utils.queue.invalidate();
      setIsOpen(false);
    },
  ...
  const markUrgentMut = trpc.queue.markUrgent.useMutation({
    onSuccess: () => {
      toast({
        title: "Marked Urgent",
        description: `${patient.patientName} priority updated.`,
      });
      utils.queue.invalidate();
      setIsOpen(false);
    },
  ...
  const cancelVisitMut = trpc.queue.cancelVisit.useMutation({
    onSuccess: () => {
      toast({
        ...
        description: `${patient.patientName}'s visit was cancelled.`,
      });
      utils.queue.invalidate();
      setIsOpen(false);
    },
  ...
  return (
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
        <DropdownMenuLabel>
          Actions: Ticket {patient.ticketNumber}
        </DropdownMenuLabel>
        <DropdownMenuSeparator />
        ...
        <DropdownMenuItem
          onClick={(e) => {
            ...
            toast({
              ...
              description: "Transfer UI coming soon.",
            });
          }}
        >
